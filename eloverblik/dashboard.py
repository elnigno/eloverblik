...
def get_meterids_in_db():
    ...
    meterids = [
        i[0] for i in conn.execute(
            """select meteringPointId from meterinfo"""
            ).fetchall()
        ]
    return meterids
...
def get_overall_consumption():
    ...
    df = conn.execute(
        ...
        index=['Meter ID', 'address'], columns='Year', values= ['Total kWh consumption']
    )
...
def chart_year_rolling_avgs(meterid=None):
    ...
    lines = (
        ...
        title='Daily kWh consumption, 7-days rolling average'
    )

    selectors = alt.Chart(df).mark_point().encode(
        x='newdate',
        opacity=alt.value(0), tooltip='strdate'
    ).add_selection(
        nearest
    )
...
def chart_hourly_consumption_bysample(df1, df2, width=0.35):
    ...
    rects1 = ax.bar(df1['hour'] - width/2, df1['tot'], width, label='Period 1', color='red')
    rects2 = ax.bar(df2['hour'] + width/2, df2['tot'], width, label='Period 2', color='blue')
...
def hourly_consumption_wrapper(meterid, years1, summer1, winter1, years2, summer2, winter2, width=0.35):
    conn = duckdb.connect(str(eloverblik.tools.datapath), read_only=True)
    df1 = get_avg_hourly_consumption(conn, meterid, years1, summer=summer1, winter=winter1)
    df2 = get_avg_hourly_consumption(conn, meterid, years2, summer=summer2, winter=winter2)
